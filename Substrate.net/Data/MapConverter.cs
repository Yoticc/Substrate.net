...
public class MapConverter
{
    ...
    public void RefreshColorCache()
    {
        ...
        {
            _labIndex[i] = RgbToLab(ColorIndex[i]);
        }
    }
    ...
    {
        Bitmap bmp = new(map.Width, map.Height, PixelFormat.Format32bppArgb);

        for (int x = 0; x < map.Width; x++)
        {
            for (int z = 0; z < map.Height; z++)
            {
                Color c = ColorIndex[map[x, z]];
                bmp.SetPixel(x, z, c);
            }
        }
