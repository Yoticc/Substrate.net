...
public class AlphaChunkManager : IChunkManager, IEnumerable<ChunkRef>
{
    ...
    private NbtTree GetChunkTree(int cx, int cz)
    {
        ...
        return nbtstr == null ? null : new NbtTree(nbtstr);
    }
    ...
    {
        return !ChunkExists(cx, cz) ? null : (IChunk)AlphaChunk.CreateVerified(GetChunkTree(cx, cz));
    }
    ...
    {
        ChunkFile cf = GetChunkFile(cx, cz);
        return cf == null ? 0 : cf.GetModifiedTime();
    }
    ...
    private class Enumerator : IEnumerator<ChunkRef>
    {
        ...
        public ChunkRef Current => _curchunk ?? throw new InvalidOperationException();
        ...
        protected static Regex _namePattern = new("c\\.(-?[0-9a-zA-Z]+)\\.(-?[0-9a-zA-Z]+)\\.dat$");
    }
}
