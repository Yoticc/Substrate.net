...
public class RegionChunkManager : IChunkManager, IEnumerable<ChunkRef>
{
    ...
    public IChunk GetChunk(int cx, int cz)
    {
        ...
        return r == null ? null : r.GetChunk(cx & REGION_XMASK, cz & REGION_ZMASK);
    }
    ...
    {
        IRegion r = GetRegion(cx, cz);
        return r == null ? null : r.GetChunkRef(cx & REGION_XMASK, cz & REGION_ZMASK);
    }
    ...
    {
        IRegion r = GetRegion(cx, cz);
        return r != null && r.ChunkExists(cx & REGION_XMASK, cz & REGION_ZMASK);
    }
    ...
    {
        IRegion r = GetRegion(chunk.X, chunk.Z);
        return r != null && r.SaveChunk(chunk);
    }
    ...
    public bool DeleteChunk(int cx, int cz)
    {
        ...
        if (r.ChunkCount() == 0)
        {
            RegionManager.DeleteRegion(r.X, r.Z);
        }
    ...
    public void RelightDirtyChunks()
    {
        ...
        Dictionary<ChunkKey, ChunkRef> dirty = new();
        ...
        while (en.MoveNext())
        {
            ChunkKey key = new(en.Current.X, en.Current.Z);
    ...
    {
        IRegion r = GetRegion(cx, cz);
        return r == null ? 0 : r.GetChunkTimestamp(cx & REGION_XMASK, cz & REGION_ZMASK);
    }
    ...
    private class Enumerator : IEnumerator<ChunkRef>
    {
        ...
        public ChunkRef Current => _x >= RegionChunkManager.REGION_XLEN ? throw new InvalidOperationException() : _chunk;
    }
}
