...
public class AnvilWorld : NbtWorld
{
    ...
    {
        return _caches.ContainsKey(dim) ? _caches[dim] : null;
    }
    ...
    {
        return dim == Dimension.DEFAULT ? "" : "DIM" + dim;
    }
    ...
    private void OpenDimension(string dim)
    {
        ...
        ChunkCache cc = new(_prefCacheSize);

        AnvilRegionManager rm = new(path, cc);
        RegionChunkManager cm = new(rm, cc);
    ...
    private AnvilWorld OpenWorld(string path)
    {
        ...
        return !File.Exists(ldat)
            ? throw new FileNotFoundException("Data file '" + _levelFile + "' not found in '" + path + "'", ldat)
            : !LoadLevel() ? throw new Exception("Failed to load '" + _levelFile + "'") : this;
    }
    ...
    {
        NBTFile nf = new(IO.Path.Combine(Path, _levelFile));
        NbtTree tree;
