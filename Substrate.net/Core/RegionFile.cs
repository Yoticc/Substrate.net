...
{
    private static Regex _namePattern = new("r\\.(-?[0-9]+)\\.(-?[0-9]+)\\.mc[ar]$");
    ...
    {
        return OutOfBounds(x, z) ? null : (Stream)new ZlibStream(new ChunkBuffer(this, x, z), CompressionMode.Compress);
    }

    public Stream GetChunkDataOutputStream(int x, int z, int timestamp)
    {
        return OutOfBounds(x, z) ? null : (Stream)new ZlibStream(new ChunkBuffer(this, x, z, timestamp), CompressionMode.Compress);
    }
