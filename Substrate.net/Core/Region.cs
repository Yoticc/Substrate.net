...
public abstract class Region : IDisposable, IRegion
{
    ...
    private bool SaveChunkTree(int lcx, int lcz, NbtTree tree, int? timestamp)
    {
        ...
        {
            IRegion alt = GetForeignRegion(lcx, lcz);
            return alt != null && alt.SaveChunkTree(ForeignX(lcx), ForeignZ(lcz), tree);
        }
    ...
    public IChunk GetChunk(int lcx, int lcz)
    {
        ...
        return !ChunkExists(lcx, lcz) ? null : CreateChunkVerifiedCore(GetChunkTree(lcx, lcz));
    }
    ...
    public bool ChunkExists(int lcx, int lcz)
    {
        ...
        {
            IRegion alt = GetForeignRegion(lcx, lcz);
            return alt != null && alt.ChunkExists(ForeignX(lcx), ForeignZ(lcz));
        }
    ...
    public bool DeleteChunk(int lcx, int lcz)
    {
        ...
        {
            IRegion alt = GetForeignRegion(lcx, lcz);
            return alt != null && alt.DeleteChunk(ForeignX(lcx), ForeignZ(lcz));
        }
        ...
        ChunkKey k = new(ChunkGlobalX(lcx), ChunkGlobalZ(lcz));
        _cache.Remove(k);
