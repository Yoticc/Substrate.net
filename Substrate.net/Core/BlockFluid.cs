...
public class BlockFluid
{
    ...
    {
        Queue<BlockKey> searchQueue = new();
        Queue<KeyValuePair<BlockKey, int>> traceQueue = new();
        Dictionary<BlockKey, int> markTable = new();
    ...
    {
        Queue<BlockKey> flowQueue = new();

        BlockKey prikey = new(x, y, z);
        ...
        while (flowQueue.Count > 0)
        {
            ...
            newflow = inFall ? inflow : inflow >= 7 ? 16 : inflow + 1;
    ...
    private void DoLava(int x, int y, int z)
    {
        ...
        while (flowQueue.Count > 0)
        {
            ...
            // Update from inflow if necessary
            newflow = inFall ? inflow : inflow >= 6 ? 16 : inflow + 2;
