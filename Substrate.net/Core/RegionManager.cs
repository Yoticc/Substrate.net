...
public abstract class RegionManager : IRegionManager
{
    ...
    private struct Enumerator : IEnumerator<IRegion>
    {
        ...
        public Enumerator(RegionManager rm)
        {
            ...
            List<string> files = new(Directory.GetFiles(rm.GetRegionPath()));
        ...
        {
            Regex R = new(".+r\\.(?<x>-?\\d+)\\.(?<y>-?\\d+)\\.(mca|mcr)", RegexOptions.None);
            Match MC = R.Match(A);
            if (!MC.Success)
                return 0;
            ...
            return AZ < BZ ? -1 : AZ > BZ ? 1 : AX < BX ? -1 : AX > BX ? 1 : 0;
        }
    }
